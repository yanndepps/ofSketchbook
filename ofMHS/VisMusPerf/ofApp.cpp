...
void ofApp::update() {
  ...
  while(osc.hasWaitingMessages()) {
    ...
    if (m.getAddress() == "/a/1/cc6/midi") {
      xVal = m.getArgAsFloat(0);
    } else if (m.getAddress() == "/b/1/cc7/midi") {
      yVal = m.getArgAsFloat(0);
    } else if (m.getAddress() == "/c/1/cc8/midi") {
...
void ofApp::trainingExamples(float x, float y, float z) {
  ...
  tempExample.output = {(double)radius,
                        (double)noiseScaleFactor,
                        (double)noiseSpeedFactor,
                        (double)noiseDisplacementFactor,
                        (double)index,
                        (double)ratio,
                        (double)carrierFreq};
...
void ofApp::modelOutput(float x, float y, float z) {
  ...
  radius = output[0];
  if ( output[0] < 10.0f ) radius = 10.0f;
